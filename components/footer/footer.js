...
const List = (props) => {
    ...
    return (
        <>
            <div style={{ fontFamily: 'Poppins-Regular' }} className="border-b-2 w-fit pb-4 mb-5">{menu.title}</div>
            {menu.content.map((menuList) => (
                <li style={{ fontFamily: 'Poppins-ExtraLight' }} className="mb-2 cursor-pointer text-[14px] text-[#ffffff] hover:text-[#EDEDED]">{menuList.title}</li>
            ))}
        </>
